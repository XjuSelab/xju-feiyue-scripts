...
- **共享云题库** `sxz-bank`（见下），按选项内容匹配防乱序。
- 防挂机保活、课程评价弹窗自动处理、强制重答开关。

## 后端 `sxz-bank/`

stdlib `http.server` + SQLite 的极简共享题库服务，Docker 部署在 huawei2，经 nginx `/sxz-bank/` 反代（`https://feiyue.selab.top/sxz-bank`）。
...
- 重建容器见 `deploy/deploy-sxz-bank.sh`，**数据卷勿删**。
